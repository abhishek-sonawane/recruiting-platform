...
function ApplicationCard({ item }) {
  ...
  const viewPdfHandle = async (fileName) => {
    const dat = await fetch(
      `${import.meta.env.VITE_BACKEND_ENDPOINT}/file/${fileName}`
    );
    console.log(dat);
  ...
  return (
    <div className=" border border-slate-300 p-10 w-full max-w-md">
        ...
        {applicationStatuses &&
          applicationStatuses.map((ApplicationItem) => {
            if (ApplicationItem == item.status) {
              return (
                <option selected value={ApplicationItem}>
                  {ApplicationItem}
                </option>
              );
            }
      ...
      <button className="px-5 text-white rounded-lg py-3 flex bg-red-400 font-bold">
        <a
          target="_blank"
          href={`${import.meta.env.VITE_BACKEND_ENDPOINT}/file/${getPdfPath(
            item.cvPDF.path.replace(/\\/g, "/")
          )}`}
        >
          {" "}
          view Resume
